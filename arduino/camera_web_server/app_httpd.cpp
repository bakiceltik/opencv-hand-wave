...
// Forwards a command string to the Arduino over the hardware UART (GPIO1/GPIO3),
// which is wired directly to the Arduino's SoftwareSerial pins. This lets the PC
// drive the robot over WiFi without the Arduino needing a USB cable.
static esp_err_t robot_cmd_handler(httpd_req_t *req) {
  char *buf = NULL;
  char value[32];
...
void startCameraServer() {
  ...
  httpd_uri_t robot_cmd_uri = {
    .uri = "/cmd",
    .method = HTTP_GET,
    .handler = robot_cmd_handler,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &robot_cmd_uri);
